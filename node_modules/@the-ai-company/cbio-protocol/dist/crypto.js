...
export function derivePublicKey(privateKey) {
    ...
    return Buffer.from(pubKeyDer).toString('base64url');
}
export function verifySignature(publicKey, signature, nonce) {
    const publicKeyDer = Buffer.from(publicKey, 'base64url');
    const signatureBuffer = Buffer.from(signature, 'base64url');
    const dataBuffer = Buffer.from(nonce);
    return verify(null, dataBuffer, { key: publicKeyDer, format: 'der', type: 'spki' }, signatureBuffer);
}

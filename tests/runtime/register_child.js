...
async function test() {
    ...
    const stored = await readSealedSecret(agent, secretName);
    ...
    const reloaded = await readSealedSecret(agentReload, secretName);
    ...
    const afterOverwrite = await readSealedSecret(agent, secretName);
